...
public class MeshTrail : MonoBehaviour
{
    ...
    [Header("Mesh Releted")]
    public float meshRetreshRate = 1.0f;            //잔상이 생성되는 시간 간격
    public float mestDestroyDelay = 3.0f;           //생성된 잔상이 사라지는 데 걸리는 시간
    ...
    private SkinnedMeshRenderer[] skinnedRenderer;  //캐릭터의 3D 모델을 렌더링하는 컴포트들
    private bool isTrailActive;                     //현재 잔상 효과가 활성화되어있는지를 확인하는 변수
    ...
    IEnumerator ActivateTrail(float timeActivated)
    {
        ...
        while (timeActivated > 0)
        {
            ...
            for (int i = 0; i < skinnedRenderer.Length; i++)                //각 메시 렌더러에 대한 잔상 생성
            {
                ...
                Destroy(gObj, mestDestroyDelay);                //일정 시간 후 잔상 제거
            }

            //다음 잔상 생성까지 대기
            yield return new WaitForSeconds(meshRetreshRate);
        }
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space) && !isTrailActive)              //스페이스바를 누르고 현재 잔상 효과가 비활성화일 때
        {
            ...
            StartCoroutine(ActivateTrail(activeTime));                      //잔상 효과 코루틴 설정
        }
    }
}
